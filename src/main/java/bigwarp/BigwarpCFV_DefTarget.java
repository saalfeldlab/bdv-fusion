...
public class BigwarpCFV_DefTarget
{
			...
			final String csvLandmarks )
	{
		...
		BigWarpData datatmp = BigWarpInit.createBigWarpData( 
				...
				new AbstractSpimData<?>[]{ dataFixedXfmed });
	...
	public static void main(String[] args)
	{
			...
			{
				System.err.println( "Must provide at least 3 inputs for moving and target image files" );
				System.exit( 1 );
			}
			...
			try
			{
				...
				BigWarp bw;
				if ( fnP.endsWith( "json" ) && fnQm.endsWith( "json" ) )
					bw = new BigWarp( genBigWarpData( fnP, fnQm, fnQmLM ), new File( fnP ).getName(), new ProgressWriterConsole() );
				else
				{
				...
				bw.getSetupAssignments().getMinMaxGroups().get( 0 ).setRange( 80, 500 );
				bw.getSetupAssignments().getMinMaxGroups().get( 1 ).setRange( 80, 500 );
